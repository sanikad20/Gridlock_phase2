"""
Gridlock Hackathon 2.0 — api/main.py
================================================
"""
...
import joblib, json, sqlite3, uuid
from datetime import datetime
import numpy as np
import pandas as pd
import os

app = FastAPI(title="Gridlock Congestion API", version="2.0")

# Allow the Streamlit frontend (and anything else) to call this API.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
try:
    ensemble_models = joblib.load(
        os.path.join(MODELS_DIR, "ensemble.pkl")
    )
    HAS_ENSEMBLE = True
...
def apply_digital_twin_adjustment(
    base_delay: float,
    base_severity: str,
    extra_barricades: int,
    close_main_road: bool,
...
def root():
    return {
        "status"  : "running",
        "message" : "Gridlock Congestion API v2.0",
...
def digital_twin(e: DigitalTwinInput):
    ...
    scenario_a = apply_digital_twin_adjustment(
        delay, severity,
        extra_barricades      = e.extra_barricades,
        close_main_road       = False,
        attendance_multiplier = 1.0
    )
    scenario_a["scenario"] = "A — Extra Barricades"

    scenario_b = apply_digital_twin_adjustment(
        delay, severity,
        extra_barricades      = 0,
        close_main_road       = True,
        attendance_multiplier = 1.0
    )
    scenario_b["scenario"] = "B — Diversion Activated"

    scenario_c = apply_digital_twin_adjustment(
        delay, severity,
        extra_barricades      = e.extra_barricades,
        close_main_road       = e.close_main_road,
        attendance_multiplier = e.attendance_multiplier
    )
...
def similar_events(e: EventInput, top_k: int = 3):
    ...
    rows = conn.execute("""
        SELECT ev.id, ev.event_cause, ev.zone, ev.corridor, ev.pred_severity,
            ev.pred_delay_mins, ev.created_at,
    ...
    if not rows:
        return {"message": "No past similar events in memory yet.", "results": []}
